...
anon_path=/home/proftpd/pub
if [[ ! -d "$anon_path" ]]; then
    mkdir -p "$anon_path"
fi
chown -R nobody:nogroup "$anon_path"
...
--name=proftpuser --uid=233 --gid=233 --home=/home/proftpd/ --shell=/bin/false
...
chown proftpd:nogroup /home/proftpd
chown -R 233:233 /home/proftpd/proftpuser

# 创建备份文件
config=/etc/proftpd/proftpd.conf
bash "$CODE_PATH/backup.sh" "$config" 'proftpd' || exit 1
